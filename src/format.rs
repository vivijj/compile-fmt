...
use crate::{
    argument::Ascii,
    utils::{assert_is_ascii, count_chars},
};
...
///
/// A format is necessary to specify for *dynamic* arguments of
/// [`compile_args!`](crate::compile_args) and related macros (i.e., for arguments that are not
/// constants). For now, the only meaningful format customization is provided for strings (`&str`).
/// All other arguments have the only available format that can be created using [`fmt()`].
///
/// # Examples
///
...
/// If clipped, the end of the string will be replaced with the specified replacer, which can be
/// empty.
///
/// # Panics
///
...
impl<T: FormatArgument> Fmt<T> {
    ...
    pub const fn capacity(&self) -> usize {
        if let Some(pad) = &self.pad {
            // Capacity necessary for an empty non-padded string (which we assume is always
            // possible).
