...
/// A format is mandatory if the argument is not a constant; e.g. if it is an argument or a local
/// variable in a `const fn`.
///
/// The value output by the macro is [`CompileArgs`](crate::CompileArgs).
///
/// # Specifying capacity
///
/// You can specify capacity of the returned `CompileArgs` by prefacing arguments with `capacity:
/// $cap,`. Here, `$cap` is a constant expression of type `usize`. The specified capacity must be
/// greater or equal that the capacity inferred from the arguments; the macro will fail with a
...
/// - [`compile_assert!`](crate::compile_assert) provides a version of the `assert!` macro with
///   support of dynamic arguments.
///
/// # Examples
///
/// ## Basic usage
...
///
/// The first argument of the macro must be a boolean value. The remaining arguments have the same
/// syntax as in the [`compile_args!`] macro.
///
/// # Examples
///
/// ```
