...
//! - The formatting logic is space-efficient; i.e., it allocates the least amount of bytes that can
//!   provably to be sufficient for all possible provided inputs. As a consequence, non-constant
...
//!
//! - [`const_panic`] provides functionality covering the guiding use case (compile-time panics). It
//!   supports more types and formats at the cost of being more complex. It also uses a different
//!   approach to compute produced message sizes.
//! - [`const_format`] provides general-purpose formatting of constant values. It doesn't seem to
//!   support "dynamic" / non-constant args.
//!
...
/// Formatted string returned by the [`compile_args!`] macro, similar to
/// [`Arguments`](fmt::Arguments).
///
/// The type parameter specifies the compile-time upper boundary of the formatted string length in
/// bytes. It is not necessarily equal to the actual byte length of the formatted string.
#[derive(Debug)]
